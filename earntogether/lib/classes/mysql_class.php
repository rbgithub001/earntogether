...
($GLOBALS["___mysqli_ston"] = mysqli_connect('localhost', 'manish_brother', 'john@2014'));
mysqli_select_db($GLOBALS["___mysqli_ston"], manish_john);
...
class mysql_func
{
	function query($slt, $table, $where=false)
	{
		...
		return mysqli_query($GLOBALS["___mysqli_ston"], "select ".$slt." from ".$table." where ".$where);
	}
	...
	function execute_query($slt, $table, $where=false, $order_by)
	{
		...
		return mysqli_query($GLOBALS["___mysqli_ston"], "select ".$slt." from ".$table." where ".$where." order by ".$order_by );
	}
...
class showDwonMem
{
	function shoDwnMem($dwnid,$tid)
	{
		function showMemX($dwnid,$tid)
		{
			...
			$data3=mysqli_query($GLOBALS["___mysqli_ston"], $quer3);
			//$le=2;
			while($arr2=mysqli_fetch_array($data3))
			{
					$idx=$arr2['user_id'];
					$data_dwn[]=$idx;
		...
		$data=mysqli_query($GLOBALS["___mysqli_ston"], $quer);
...
class showDwonMemFV_new1
{
   public $data_dwn=array();
	...
	function shoDwnMemFV_new_level1($dwnid,$tid,$user_plan)
	{
		...
		$simple=mysqli_query($GLOBALS["___mysqli_ston"], $s);
		$count=mysqli_num_rows($simple);
		return $count;
	}
...
class Commission
{
	function _upgrade_sponser_position($user_id)
	{
		...
		if($count)
		{
			if($count>=100 && $row['user_plan']=='Representative')
			{
				...
				mysqli_query($GLOBALS["___mysqli_ston"], $upgrade);
			}
	...
	function personal_purchasing($user_id)
	{
		...
		$res_pv=mysqli_query($GLOBALS["___mysqli_ston"], $sql_pv);
		$row_pv=mysqli_fetch_assoc($res_pv);
		...
		if($count) // start if 001
		{
			...
			if($row['user_rank']) // start if 002
			{
				...
				else if($row['user_rank']==6 && $this->team_purchasing($user_id)>150000)
				{
					...
					if($flag)
					{ 
					mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=7, user_plan='PLATINUM' where user_id='$user_id'");
					mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=7,user_id='$user_id'");
				...
				else if($row['user_rank']==5 && $this->team_purchasing($user_id)>100000)
				{
					...
					if($flag)
					{ 
					mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=6, user_plan='ELITE PRESIDENT' where user_id='$user_id'");
					mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=6,user_id='$user_id'");
				...
				else if($row['user_rank']==4 && $this->team_purchasing($user_id)>50000)
				{
					...
					if($flag)
					{ 
					mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=5, user_plan='PRESIDENT' where user_id='$user_id'");
					mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=5,user_id='$user_id'");
				...
				else if($row['user_rank']==3 && $this->team_purchasing($user_id)>25000)
				{
					...
					if($flag)
					{
					mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=4, user_plan='ELITE DIRECTOR' where user_id='$user_id'");
					mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=4,user_id='$user_id'");
				...
				else if(($row['user_rank']==1 || $row['user_rank']==2) && $this->team_purchasing($user_id)>6000)
				{
					...
					if($flag)
					{
						mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=3, user_plan='DIRECTOR' where user_id='$user_id'");
						mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=3,user_id='$user_id'");
			...
			else // start else 003
			{
				...
				if($curtime<=$endtime) // start if 004
				{
					...
					if($row_pv['amount']>=300) // start if 005
					{
						...
						mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=1, user_plan='Excel Express Manager' where user_id='$user_id'");
						mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=1,user_id='$user_id'");
				...
				else  // start else 004
				{
						...
						if($curtime1<=$endtime1) // start if 006
						{
							...
							//echo $sql_pv1;exit;
							$res_pv1=mysqli_query($GLOBALS["___mysqli_ston"], $sql_pv1);
							$row_pv1=mysqli_fetch_assoc($res_pv1);
							if($row_pv1['amount']>=300) // start if 007
							{
								...
								$from_date_m=date('Y-m-01');
								$to_date_m=date('Y-m-t');
								mysqli_query($GLOBALS["___mysqli_ston"], "update registration set user_rank=2, user_plan='Excel Manager' where user_id='$user_id'");
								mysqli_query($GLOBALS["___mysqli_ston"], "insert into user_rank_achieve set rank_id=2,user_id='$user_id'");
								if($this->monthly_autoship($user_id,$from_date_m,$to_date_m)>=20)
								{
	...
	function commulative_from_one_leg($user_id)
	{
		...
		$res_ref=mysqli_query($GLOBALS["___mysqli_ston"], $sql_ref);
		$count_ref=mysqli_num_rows($res_ref);
		$start=$count_ref;
		$flag=1;
		$team_purchasing=$this->team_purchasing($user_id);
	...
	function bonus_distribution($user_id,$pep,$invoice_no,$invoice_amt)
	{
		...
		{*/
			...
			$res_check_history=mysqli_query($GLOBALS["___mysqli_ston"], $sql_check_history);
			$count_check_history=mysqli_num_rows($res_check_history);
			...
			{*/
				...
				{*/
					...
					{*/
						...
						$res_level=mysqli_query($GLOBALS["___mysqli_ston"], $sql_level);
						...
						if($row_level['amount']<105)
						{
							...
							$res_up_rank=mysqli_query($GLOBALS["___mysqli_ston"], $sql_up_rank);
							$row_up_rank=mysqli_fetch_assoc($res_up_rank);
							...
							$level_com="INSERT INTO `level_income_bonus` (`l_id`, `invoice_no`, `purcheser_id`, `income_id`, `level`, `commission`, `date`, `payout_date`, `status`,`paid_status`, `invoice_amt`, `com_percent`, `invoice_bv`, `commission_bv`, `closing`,`credit_debit_id`,`section`,`remark`,`user_level`) VALUES (NULL, '$invoice_no', '$user_id', '$ref_id1', '$cnt', '$quick_bonus', curdate(), '', '0','0', '$invoice_amt', '35', '$pep', '', '','$cdid','Quick Start Bonus','Get Quick Start Bonus from user $user_id of invoice $invoice_no','$user_level');";
							//echo $level_com;exit;
							mysqli_query($GLOBALS["___mysqli_ston"], $level_com);
						}
						/*else
						{
	...
	if($user_rank>0)
	{
		while($r!='cmp')
		{
			...
			if($r!='')
			{
				$sltnom=mysqli_query($GLOBALS["___mysqli_ston"], "select * from registration where user_id='$r'");
				$fetchnom=mysqli_fetch_array($sltnom);
	...
	function commission_distribute($ref,$nom,$user_id,$plan_name,$user_level)
	{
		...
		while($l!='cmp')
		{
			...
			//echo "select * from registration where user_id='$l'";
				$sltnom=mysqli_query($GLOBALS["___mysqli_ston"], "select * from registration where user_id='$l'");
				$fetchnom=mysqli_fetch_array($sltnom);
	...
	function _update_group_volume($user_id,$tot,$product_volume,$invoice_no)
	{
		...
		$res_user=mysqli_query($GLOBALS["___mysqli_ston"], $sql_user);
		...
		mysqli_query($GLOBALS["___mysqli_ston"], "update final_bv set personal_purch=(personal_purch+'$tot'),ppurch_befr_affiliation=(ppurch_befr_affiliation+'$tot'),bv=(bv+'$product_volume'),monthly_tpv=(monthly_tpv+'$tot'),monthly_ppv=(monthly_ppv+'$tot'),tbv=(tbv+'$product_volume'),rank_pv=(rank_pv+$tot) where user_id='$user_id'");
		mysqli_query($GLOBALS["___mysqli_ston"], "insert into purchase_history set invoice_no='$invoice_no',purcheser_id='$user_id', income_id='$user_id',level=0,commission='',date=curdate(),invoice_amt='$tot',com_percent='',invoice_bv='$product_volume',closing='',status=0"); 
		//Entry for level 
		$l=1;
		while($ref!='cmp')
		{
			$selectnompos=mysqli_query($GLOBALS["___mysqli_ston"], "select * from registration where user_id='$ref'");
			...
			if($com_id!='cmp' && $com_id!='')
			{
				...
				mysqli_query($GLOBALS["___mysqli_ston"], "insert into purchase_history set invoice_no='$invoice_no',purcheser_id='$user_id', income_id='$com_id',level=$l,commission='',date=curdate(),invoice_amt='$tot',com_percent='',invoice_bv='$product_volume',closing='',status=0"); 
				mysqli_query($GLOBALS["___mysqli_ston"], "update final_bv set team_purch=(team_purch+'$tot'),tpurch_befr_affiliation=(tpurch_befr_affiliation+'$tot'),bv=(bv+'$product_volume'),tbv=(tbv+'$product_volume'),bv=(bv+'$product_volume'),tbv=(tbv+'$product_volume'),monthly_tpv=(monthly_tpv+'$tot'),monthly_ppv=(monthly_ppv+'$tot'),tbv=(tbv+'$product_volume'),rank_pv=(rank_pv+$tot) where user_id='$com_id'");
	...
	function _update_group_volume_mb($user_id,$tot,$product_volume,$invoice_no)
	{
		...
		mysqli_query($GLOBALS["___mysqli_ston"], "insert into purchase_history_mb set invoice_no='$invoice_no',purcheser_id='$user_id', income_id='$ref',level=0,commission='',date=curdate(),invoice_amt='$tot',com_percent='',invoice_bv='$product_volume',closing='',status=0"); 
		//Entry for level 
	}
	function merchant_bonus_distribution($user_id,$total_volume1,$invoice_no,$tot)
	{
		...
		mysqli_query($GLOBALS["___mysqli_ston"], "insert into level_income_total set invoice_no='$invoice_no',purcheser_id='$user_id', income_id='$ref',level=0,commission='$commission',date=curdate(),invoice_amt='$tot',com_percent='10',invoice_bv='$total_volume1',closing='',status=0,remark='Get Merchant Bonus'"); 
		//Entry for level 
	}
}
		...
		if($count)
		{
			while($row=mysqli_fetch_assoc($res))
			{
				...
				if($count_user || $income_id=='guru')
				{
					...
					mysqli_query($GLOBALS["___mysqli_ston"], $insert_wallet);
					...
					mysqli_query($GLOBALS["___mysqli_ston"], $update_com_paid);
					//start update commission to paid
