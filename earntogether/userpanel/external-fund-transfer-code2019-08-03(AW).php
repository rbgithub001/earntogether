...
if(isset($_POST['update'])){
  ...
  if( !empty($_POST['amounts']) && !empty($_POST['user']) && !empty($_POST['t_password']))
  {
    if( $_POST['amounts'] >= $min_amount )
    {

      $newcode=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select t_code from user_registration where user_id='".$f['user_id']."'"));

      if($newcode['t_code']=='')
      {
        $_SESSION['err']="Please enter correct transaction password!";
      ...
      $walletfrom=$_POST['walletfrom'];
        ...
        if( $args_wallet['amount'] >= $total_charge)
        {
          ...
          if(mysqli_num_rows($rst_receiver) > 0 ){
            ...
            $sql_cr_Dr .= "ewallet_used_by = '".$wall."',";
...
$_SESSION['err']="Please enter correct transaction password..!";
		...
		$_SESSION['err']="Please enter min ".$min_amount." USD transfer to user wallet";
            header("location:external-fund-tranfer.php");
                  exit();
    }
