  ...
  if( !empty($_POST['amounts']) && !empty($_POST['user']) && !empty($_POST['password']))
  {
    if( $_POST['amounts'] >= $min_amount )
    {
      ...
      $walletfrom=$_POST['walletfrom'];
        ...
        // $rwallet="roi_e_wallet";
          $wallets='working_e_wallet';
          $rwallet='working_e_wallet';
            $msg='Working Wallet';
        ...
        if( $args_wallet['amount'] >= $total_charge)
        {
          ...
          $ttype="Chips Transfer";
          $walleting="Chips Wallet";
       ...
       $_SESSION['err']="Fund transferred successfully..!";
        ...
        exit();
			 ...
			 $_SESSION['err']="Please enter correct User ID or Username!!";
            header("location:external-fund-tranfer1.php");
            exit();
          }
          
        }else{
			$_SESSION['err']="Insufficient fund in your Chips Wallet..!";
                  header("location:external-fund-tranfer1.php");
                  exit();
        }
...
$_SESSION['err']="Please enter correct login password..!";
		...
		$_SESSION['err']="Please enter min ".$min_amount." Chips transfer to user wallet";
            header("location:external-fund-tranfer1.php");
                  exit();
    }
