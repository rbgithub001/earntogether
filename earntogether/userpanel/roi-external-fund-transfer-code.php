  ...
  if( !empty($_POST['amounts']) && !empty($_POST['user']) && !empty($_POST['password']))
  {
    if( $_POST['amounts'] >= $min_amount )
    {
      ...
      if($newcode['password']=='')
      {
        ...
        header("location:roi-external-fund-tranfer.php");
        exit;
      }
      ...
      if($_POST['password']==$pwd)
      {
       ...
       // $walletfrom=$_POST['walletfrom']; 
        $walletfrom=$wallet; 
         if ($walletfrom=='roi_e_wallet') {
            $wallets='roi_e_wallet';
            $msg='ROI Wallet';  
         
         }/*elseif($walletfrom=='b_wallet') {
            $wallets='b_wallet';
            $msg='B Wallet';
         
         }elseif ($walletfrom=='t_wallet') {
            $wallets='t_wallet';
            $msg='T Wallet';
         }
         
         elseif ($walletfrom=='rmb_wallet') {
            $wallets='rmb_wallet';
            $msg='RMB Wallet';
         }*/
         $rwallet="final_e_wallet";
         $checkdown=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from matrix_downline where down_id='$userid' and income_id='".$rec."'"));
         if ($checkdown >0) {
            $admin_amount=0;
            $admin_charge=0.00;
         }else{
