...
// $date= date('Y-m-d',strtotime($date1.'+1 Day'));
 ...
 $resdt=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `lifejacket_subscription` where invest_type='Sharia Investment'");
...

{
...

  {
     ...
     $data=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from credit_debit where ttype='Roi Income' AND receive_date='".date('Y-m-d')."' and TranDescription='Sharia Investment'"));
    if($data<=0)
    {
...
mysqli_query($GLOBALS["___mysqli_ston"], "update lifejacket_subscription set pb=(pb+$rwallet) where id='".$last_life['id']."'");
  ...
  mysqli_query($GLOBALS["___mysqli_ston"], "insert into credit_debit values(NULL,'$invoice_no','$uid','$rwallet','0','0','$uid','123456','$date','Roi Income','Sharia Investment','$pb','$lifejacket_id','$invoice_no','$first1','0','ROI Wallet',CURRENT_TIMESTAMP,'$urls')");  
     }else{
      header("location:edit_daily_roi1.php?msg=Not Allowed,You have already Closed!!");exit;
...
header("location:roi-income-report-daily1.php");exit;
}
                ...
                <div class="col-sm-6">
                    <section class="panel" style="background-color: #f1bebe;">
                        <header class="panel-heading">
                            Islamic investment Closing<span style="float:right;color:red;"><?php echo @$_GET['msg'];?></span>
                        </header>
