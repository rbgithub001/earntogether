...
if($frm!='' && $til!='')
{
  $query123[]=" withdraw_request.posted_date between '$f1' and '$f2' ";
}
 if($username!='')
  {
...
if ($paytype!='') {
 $query123[]=" withdraw_request.request_amount= '$paytype'"; 
}
if ($memtype!='') {
                    ...
                    <div class="col-sm-12">
                        <section class="panel">
                            <header class="panel-heading ">
                                  Check Downline Withdrawal Report
...
<?php 
$i=1;
$data=mysqli_query($GLOBALS["___mysqli_ston"], "select level_income_binary.down_id,level_income_binary.income_id,level_income_binary.leg,level_income_binary.l_date,sum(withdraw_request.request_amount) as tt,withdraw_request.withdraw_wallet,user_registration.username,user_registration.user_id from level_income_binary inner join withdraw_request on level_income_binary.down_id=withdraw_request.user_id inner join user_registration on withdraw_request.user_id=user_registration.user_id and $qrt group by withdraw_request.user_id");
    while($data1=mysqli_fetch_array($data)){ ?>
                            <tr style="text-align:center;">
                                ...
                                <td><?php  echo $data1['tt'];?>
                                </td>
                                 <td>
                                    <?php echo $data1['l_date']; ?>
                                </td>
                                <td>
                                <?php  
