...
<input type="submit" name="submits" value="Submit" class="btn btn-success">           
    ...
    <div class="total_invoice"  id="printArea">
                            ...
                            <?php 
                                  ...
                                  //$data=mysqli_query($GLOBALS["___mysqli_ston"], "select * from puc_credit_debit where ttype='Admin Commission' $query123");
                                  $data=mysqli_query($GLOBALS["___mysqli_ston"], "select * from puc_credit_debit where ttype='Admin Commission' $query123 GROUP BY sender_id");
                                  while($data1=mysqli_fetch_array($data)){ 
                                  
                                  $invoiceamnt=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select sum(Cause) as invoiceamnt,sum(credit_amt) as creditamnt from puc_credit_debit where ttype='Admin Commission' and sender_id='".$data1['sender_id']."'"));
                                  $total=$total+$invoiceamnt['creditamnt'];
                                  $totalcomision=$totalcomision+$invoiceamnt['invoiceamnt'];
                                  ?>
                            <tr style="text-align:center;">
                            <?php $data11=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from poc_registration where user_id='".$data1['sender_id']."'"));?>
                                ...
                                <td>
                                    <?php echo $data1['transaction_no'];?>
                                </td>
                                <td><?php echo $data1['sender_id'];?></td>
                                <td><?php echo $data11['first_name']." ".$data11['last_name'];?></td>
                                <td><?php //echo $data1['Cause'];
                                echo $invoiceamnt['invoiceamnt'];
                                ?></td>
                                
                                <td><?php echo $data1['Remark'];?></td>
                                <td><?php //echo $data1['credit_amt'];
                                echo $invoiceamnt['creditamnt'];
                                ?></td>
                               <!-- <td><?php echo $data1['invoice_no'];?></td>-->
                                <td><?php echo $data1['receive_date'];?></td>
                                <!--<td><?php // if($data1['status']==0) echo "Unpaid"; else echo "Unpaid";?></td>-->
                                 <td><a style="color:#34379e" href="vendorwise-commission-details.php?sender_id=<?php echo $data1['sender_id'];?>">View Details</a></td>
                            </tr>
                            <?php 
                            $i++;
                            }
                      ...
                      <td><span><h4><?php echo $totalcomision; ?></h4></span></td> 
                      <td>&nbsp;</td>
                      <td>&&</td>
                      <td></td>
