...
if($User!='' && $amount!='' && $package!='')
{
 ...
 $sqlqw1=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from status_maintenance where id=1"));
	...
	if($_POST['package']==1)
	{
		...
		{
		header("location:email_search_report.php?msg=Amount should be match with the package selected");
		exit;
	    }
	}
	...
	{
		$package=0;
		header("location:email_search_report.php?msg=Select package");
		exit;
	}
...
header("location:email_search_report.php?msg=Status Updated!");
exit;
}
else
{
	header("location:email_search_report.php?msg=Unable to update now!");
	exit;
}
