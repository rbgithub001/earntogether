...
$prop_com_perc= $_POST['prop_com_perc'];
 ...
 if($ccm>0)
  ...
  //$ref= $refquery['ref_id'];
    ...
    $comm=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from user_rank_list where id='8' "));
    
    $spcs=$comm['level_per'];
    
	$comm_amt = $saleamount*$spcs/100;
	//$spcs = 5;
	    ...
	    //mysqli_query($GLOBALS["___mysqli_ston"], "update final_e_wallet set amount=(amount+$finalpay) where user_id='".$ref."'");
         $quesale="INSERT INTO `credit_debit`(`transaction_no`, `user_id`, `credit_amt`, `debit_amt`, `admin_charge`, `receiver_id`, `sender_id`, `receive_date`, `ttype`, `TranDescription`, `Cause`,`commis_amt`, `Remark`, `invoice_no`, `product_name`, `status`, `ewallet_used_by`,  `current_url`, `package_id`, `percent`, `total_invesment_amount`) 
                VALUES ('$inv','$ref','$finalpay','0','5','$ref','$sellerid','$date','$ttyperef','Earn Referral Income from $sellerid','0','$comm_amt','$ttyperef','$inv','0','0','EWallet','0','$spcs','$spcs','$saleamount')";
...
$data_level1=mysqli_query($GLOBALS["___mysqli_ston"], "select * from matrix_downline where down_id='$sellerid' ");
  while($data_level12=mysqli_fetch_array($data_level1))
  {
    ...
    //$comm=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from user_rank_list"));
   ...
   // $totals=$totalamount*$spc/100;
    ...
    if ($comm_amt>0 && $spc>0 ) {
               ...
               //mysqli_query($GLOBALS["___mysqli_ston"], "update final_e_wallet set amount=(amount+$finalpay) where user_id='".$income_id."'");
                $quesale="INSERT INTO `credit_debit`(`transaction_no`, `user_id`, `credit_amt`, `debit_amt`, `admin_charge`, `receiver_id`, `sender_id`, `receive_date`, `ttype`, `TranDescription`, `Cause`,`commis_amt`, `Remark`, `invoice_no`, `product_name`, `status`, `ewallet_used_by`,  `current_url`, `package_id`, `percent`, `total_invesment_amount`) 
                VALUES ('$inv','$income_id','$finalpay','0','5','$income_id','$sellerid','$date','$ttype2','$ttype2','$relecom','$comm_amt','$ttype2','$inv','0','0','EWallet','$relcomper','$userLevel','$spc','$saleamount')";
...
<input type="hidden" name="randm" name="randm" value = "<?php echo htmlentities($salt);?>" />   
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-10">
                                         <input type="text" name="country" class="form-control" id="exampleInputAddress" value="<?php echo $cust['country']?>" readonly>
                                    </div>
                                </div>
                                <div class="form-group">
