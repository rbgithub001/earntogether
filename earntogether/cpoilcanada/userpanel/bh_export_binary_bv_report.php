...
exit;*/
$select = mysqli_query($GLOBALS["___mysqli_ston"], "select * from manage_bv_history where income_id='$userid' order by id desc");
...
if(mysqli_num_rows($select)>0)
{
	...
	{
		$data11=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from user_registration where user_id='".$row['downline_id']."'"));
		...
		$content .= escape_csv_value($row['downline_id']).",";
		$content .= escape_csv_value($row['bv']).",";
		$content .= escape_csv_value($row['position']).",";
...
$title .= "Sr No., Downline Name, Downline Id, BV, Position, Level, Remark, Date, Status"."\n";
echo $title;
echo $content;
