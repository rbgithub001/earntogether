 ...
 if($f['status']=='0')
{
  ...
  header("location:new-transaction-password.php?user_id=$userid&token=$token&status=$status&msg=Confirm Transaction Password does not match !");
     exit;  
  }
...
mysqli_query($GLOBALS["___mysqli_ston"], "update user_registration set  t_code='$databasepwd1' where (user_id='$userid' || username='$userid')");
mysqli_query($GLOBALS["___mysqli_ston"], "update forgot_password set  status=1 where userid='$userid' and token='$token'");
header("location:login.php?msg=Transaction Password Updated Successfully!");
}
else{
  header("location:forgot_transaction.php?msg=Please try Again !");
exit;
}
}
 ?>
