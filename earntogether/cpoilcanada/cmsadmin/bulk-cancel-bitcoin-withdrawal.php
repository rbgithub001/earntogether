                    ...
                    <div class="col-sm-12">
                        <section class="panel">
                            ...
                            <?php
                                ...
                                if(isset($_POST['Cancel']) && !empty($_POST['Cancel']) && ($_POST['Cancel'] == 'Cancel Fund') ){

                                    $total_lists = count($_POST['list']);

                                    foreach ($_POST['list'] as $key => $request_id) {
                                        ...
                                        if(count($data)){
                                            ...
                                            $user_data = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT u.username, u.user_id, u.email FROM `withdraw_request` as w INNER JOIN user_registration AS u ON (u.user_id = w.user_id) WHERE w.id = '".$request_id."'"));
                                            ...
                                            color: rgb(38, 38, 38);width: 100%;margin-top: 24px;">
                                              ...
                                              $refunded_arr[] = $request_id;

                                        }else{
                                            $failed_arr[] = $request_id;
                                        }
                                    }

                                    if(count($refunded_arr) == $total_lists){
                                        // header("location: open-withdrawal-request.php?msg1=All selected amount refunded successfully!");
                                        ?>
                                        
                                        <script type="text/javascript">
                                        alert("All selected amount refunded successfully!");
                						 window.location.href='open-withdrawal-request.php';
                						</script> 	
                                        
                                        <?php
                                        exit();
                                    }else{
                                        $count = count($refunded_arr);
                                        // header("location: open-withdrawal-request.php?msg1=Only ".$count." amount refunded successfully!");
                                        ?>
                                        
                                        <script type="text/javascript">
                                            alert("Only <?php echo $count; ?> amount refunded successfully!");
                						 window.location.href='open-withdrawal-request.php';
                						</script> 	
