...
$_SESSION['rand'] = mt_rand(1111111,9999999); 
  ...
  $id=$_GET['id'];
  $sql=mysqli_query($GLOBALS["___mysqli_ston"], "select * from projects where project_id='".$id."'");
...
$project_name=$_POST['project_name'];
 ...
 if(!empty($pdffile)){
       move_uploaded_file($_FILES['pdffile']['tmp_name'],"project_pdf/".$pdffile);
   $pdffile =$pdffile;
...
$str="update projects set category_id='".$category."', project_name='".$project_name."',pdf_link='".$pdffile."' WHERE project_id='$id'";
$res=mysqli_query($GLOBALS["___mysqli_ston"], $str)or die("error");

if($res)
{
header("location:edit_project.php?id=". $id."&msg=Project Information Updated Successfully !");
}
                        ...
                        <div class="panel-body">
                            <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
                             <div class="form-group">
                                    <label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Project Category </label>
                                    <div class="col-lg-10">
                                       <select name="category" class="form-control" id="category" onChange="subCategory(this.value)" >
                                    ...
                                    <?php $action=mysqli_query($GLOBALS["___mysqli_ston"], "select * from categories where status=0");
                                       ...
                                       ?>
                                       <option value="<?php echo $data['category_id'];?>" <?php if( $data['category_id']==$row['category_id']){ echo 'selected';  } ?>><?php echo $data['name'];?></option>
                                       <?php } ?>
                                       </select>    
                                ...
                                <div class="form-group">
                                    <label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Project Name</label>
                                    <div class="col-lg-10">
                                       <input required type="text" name="project_name" class="form-control" value="<?php echo $row['project_name'];?>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Project PDF</label>
                                    <div class="col-lg-10">
                                       <input  type="file" name="pdffile" class="form-control" >
                                       <a href="project_pdf/<?php echo $row['pdf_link'];?>" target="_blank"><?php echo $row['pdf_link'];?></a>
                                    </div>
