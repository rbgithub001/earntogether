...
$title = '';
 $ii=1;
			     $sql="select * from user_registration where (issued='YES' || issued='UPG') order by id desc ";
...
$content .= escape_csv_value($ii).",";
	...
	$content .= escape_csv_value($row['issued']).",";
...
$title .= "Sr.No ,Userid,Username,Member Name,Issued,Issued date,Product Type   "."\n";
echo $title;
echo $content;
